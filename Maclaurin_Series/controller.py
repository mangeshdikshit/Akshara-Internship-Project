...
from .Deriving_from_definitions import generate as generate_maclaurin_definition
from .for_common_functions import generate as generate_maclaurin_expansion
from .Identifying_patterns import generate as generate_maclaurin_patterns

levels = [f"Level {i}" for i in range(1, 8)] # Levels 1 to 7

def generate_all_maclaurin_questions_controller(level_str: str, total_questions: int):
    ...
    output_lines = []
    output_lines.append(f"# Maclaurin Series Questions: {level_str} (Total: {total_questions})\n")

    # Define the sub-topic generators and their titles - using the 3 provided modules
    sub_topic_generators = [
        ("Deriving Maclaurin Series from Definition", generate_maclaurin_definition),
        ("Maclaurin Series Expansion of Known Functions", generate_maclaurin_expansion),
        ("Identifying Maclaurin Series Coefficient Patterns", generate_maclaurin_patterns),
    ]

    num_sub_topics = len(sub_topic_generators) # This will be 3
...
def run_topic_main():
    ...
    print("Available Levels for Maclaurin Series Questions:")
    ...
    return generate_all_maclaurin_questions_controller(selected_level_str, total_questions)

from .Error_estimation import generate as generate_taylor_error
from .Expanding_known_functions import generate as generate_taylor_expansion_terms
from .Taylor_expansion_formula import generate as generate_taylor_expansion_order 


levels = [f"Level {i}" for i in range(1, 8)] # Levels 1 to 7

def generate_all_taylor_questions_controller(level_str: str, total_questions: int):
    ...
    output_lines = []
    output_lines.append(f"# Taylor Series Questions: {level_str} (Total: {total_questions})\n")

    # Define the sub-topic generators and their titles
    sub_topic_generators = [
        ("Taylor Series Error Estimation (Lagrange Remainder)", generate_taylor_error),
        ("Taylor Series Expansion (Number of Terms)", generate_taylor_expansion_terms),
        ("Taylor Series Expansion (Order of Polynomial)", generate_taylor_expansion_order), # Using the second identical file
...
def run_topic_main():
    ...
    print("Available Levels for Taylor Series Questions:")
    ...
    return generate_all_taylor_questions_controller(selected_level_str, total_questions)

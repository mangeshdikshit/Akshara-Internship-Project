...
def generate_expression(vars, pos=True, neg=True, frac=False):
    ...
    for var in vars:
        ...
        expr += coeff * var
    outer_const = get_random_coeff(pos, neg, frac)
    return outer_const * expr

def generate_question_const_multiple(level):
    config = LEVELS[level]
    dims = config['dims']
    vars_map = [i, j, k][:dims]
    ...
    # Template variety
    templates = [
        "Evaluate using the linearity property (constant multiple):",
        "Find the result by factoring out the constant:",
        "Use constant multiple rule of summations to simplify:",
        "Apply linearity of summation to compute:",
        "Simplify the sum using its linear structure:"
    ]
...
def generate_questions(level, num_questions):
    ...
    for _ in range(num_questions):
        question, answer = generate_question_const_multiple(level)
